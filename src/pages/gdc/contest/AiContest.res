let contents = {
  ...
  Il ${Strong(Str("tema"))} scelto per la GDC di quest'anno ${Str(
      "è",
    )} riassunto nel titolo "noMADE" in doppia accezione italiana e inglese (nomade e non fatto/abbandonato).

  ${NL}

  Caratteristica del nomadismo ${Str(
      "è",
    )} l'esistenza transitoria dell'individuo attraverso gli spazi ed i luoghi.

  ${NL}

  In questo senso, il contest si propone di esplorare il progetto di architettura attraverso i temi dell'effimero.${NL} ${Strong(
      Str("\"Effimero\""),
    )} ${Str(
      "è",
  ...
  ${Str("•")} Il concorso ${Str("è")} ${Strong(
      Str("aperto a tutti"),
    )} gli studenti e a tutti gli appassionati di architettura, designer, artisti e creativi in generale.${NL}

  ${Str("•")} ${Strong(
      Str({escaper`Non ${"è"} richiesta alcuna qualifica`}),
    )} specifica ma solo passione e curiosit${Str("à")}.${NL}

  ${Str("•")} Si pu${Str("ò")} partecipare ${Strong(Str("individualmente o in team"))} (${Strong(
      Str("massimo 2"),
    )} membri per team).
  ...
  Indirizzo email: ${Link(
      "mailto:asap.arkimastria@yahoo.it",
      Strong(Str("asap.arkimastria@yahoo.it")),
    )}
